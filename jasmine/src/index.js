...
function buttonClicked(event) {
    ...
    if (event.target.innerHTML == "=") {
        equationString = respondToEqualsClick(equationString);
...
function respondToEqualsClick(myEquationString) {
    // console.log(eval(equationString));
    //secondaryDisplay.innerHTML = myEquationString + "=";
    let answer = calculateAnswer(myEquationString);
    // Let eval provide the answer to perform order of operations correctly
    console.log(answer);
    //display.innerHTML = answer;
    myEquationString = String(answer);
    // Assign the answer to the equation string
    termQueue.unshift(termString);
    termQueue = [];
    termQueue.unshift(answer);
    termString = String(answer);
    equationString = myEquationString;
    //display.style.animation = "mymove 0.5s 1";
    return myEquationString;
}

function calculateAnswer(equationString) {
    let x = eval(equationString);
    //alert(typeof (x));
...
function clear() {
    ...
    return { equationString: equationString, termString: termString, termQueue: termQueue, buttonQueue: buttonQueue, displayString: displayString, secondaryDisplay: secondaryDisplayString };
